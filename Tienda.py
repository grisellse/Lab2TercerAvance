...
class VVentas(QWidget):
    def __init__(self):
        ...
        self.tab_productos = QTableWidget(0, 5)
        self.tab_productos.setHorizontalHeaderLabels(["Id", "Nombre", "Descripción", "Precio", "Cantidad"])
    ...
    def generar_factura(self):
        ...
        cliente = {
            'nombre': 'Cliente Ejemplo',  # Aquí podrías obtener el nombre del cliente de un campo de entrada
            'direccion': 'Direccion Ejemplo',  # Similar para la dirección
            'telefono': '999-9999'  # Y el teléfono
        }
        
        productos = []
        for row in range(self.tab_productos.rowCount()):
            id_producto = self.tab_productos.item(row, 0).text()
            nombre = self.tab_productos.item(row, 1).text()
            precio = float(self.tab_productos.item(row, 3).text())
            cantidad = int(self.tab_productos.item(row, 4).text())
            productos.append({'nombre': nombre, 'cantidad': cantidad, 'precio': precio})

        factura_pdf = FacturaPDF(vendedor, cliente, productos)
        factura_pdf.generar_factura_pdf()
        
        self.mostrar_mensaje("Factura generada correctamente.")
...
def generar_factura(self):
    ...
    cliente = {
        'nombre': self.cliente_input.text(),
        'direccion': self.direccion_input.text(),
        'telefono': self.telefono_input.text()
    }
    
    productos = []  
    for item in self.lista_productos.selectedItems():
        nombre = item.text()  
        cantidad = 1  
